...
| Documentos de auditoría faltantes | Verifica que el servicio `audit-worker` esté arriba y consumiendo `fleetcore.audit`; si RabbitMQ cae, los eventos se guardan en `audit_outbox` y `AuditOutboxRelayService` los republica al recuperarse el broker; si Mongo falla en el consumo, el consumidor reintenta vía `fleetcore.retry` y aparca en `fleetcore.dead-letter`. La escritura síncrona directa a Mongo es solo la última red de seguridad. |
