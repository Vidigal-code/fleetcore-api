...
- **Unit tests** (`frontend/tests/unit`) rely on React Testing Library to verify the auth slice, `ConfirmDialog` and `SelectField`. (Jest `testMatch` allows `.spec.ts` and `.spec.tsx`.)
- **Playwright** (`frontend/tests/e2e`) covers the login flow against the running API.
