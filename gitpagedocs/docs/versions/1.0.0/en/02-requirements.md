...
- **Automated tests**: backend unit/integration (vehicle search/ordering)/e2e suites in `backend/tests`; frontend unit (auth slice, ConfirmDialog, SelectField) + Playwright login spec in `frontend/tests`.
- **Resilience**: retry/circuit breaker policies (cockatiel) from `backend/src/shared/resilience/resilience.service.ts` wrap messaging and audit operations.
